...
在 wechat-article-spider skill 通用抓取逻辑之上，按扁平结构输出：
  CS的陋室/
    ├── images/                       ← 公共图片目录
    └── YYYYMMDD-标题.md              ← md 直接放顶层（无子文件夹）

命名遵循全局规范：日期前缀用文章发布时间（非抓取时间）。

用法:
    python save_csloushi_article.py <文章URL> [--force]
...
from html import unescape as html_unescape  # 解码标题中的 HTML 实体（如 &amp; &nbsp;）
...
BASE_DIR = os.path.normpath(
    r"E:\我的科研1\003 大模型LLM\000 LLM Agent学习资料\004 微信公众号\CS的陋室"
)
...
def main():
    if len(sys.argv) < 2:
        print("❌ 用法：python save_csloushi_article.py <文章URL> [--force]")
        sys.exit(1)
    ...
    article["title"] = html_unescape(article["title"])  # &amp; → & 等
