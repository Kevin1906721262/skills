...
def main():
    ...
    # 5. 保存文件
    # 用发布时间 + 标题生成文件名
    safe_title = article['title'][:80].replace('/', '_').replace('\\', '_')
    # Remove problematic chars but keep Chinese punctuation
    safe_title = "".join(c for c in safe_title if c.isalnum() or c in ' -_，。！？：；（）《》、+-')
    safe_title = safe_title.strip()
    if not safe_title:
        safe_title = "wechat-article"
    
    # Prepend publish date if available
    if article['publish_date']:
        date_prefix = article['publish_date'].replace('-', '')
